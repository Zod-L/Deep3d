...
import os
from mesh import load_obj_mesh, save_obj_mesh_with_rgb
...
class ParametricFaceModel:
    def __init__(self, 
        ...
        if (cartoon_weight_shape is not None and "mean_cartoon_shape" in model):
        ...
        if (cartoon_weight_tex is not None and "mean_cartoon_texture" in model):
    ...
    def compute_for_render(self, coeffs, im, skin_mask):
        """
        Return:
        ...
        skin_mask_3d = F.grid_sample(skin_mask, face_proj.unsqueeze(2).to(skin_mask.device), mode="nearest").squeeze(3).permute(0,2,1)
        # im = F.grid_sample(im, face_proj.unsqueeze(2).to(im.device), mode="nearest").squeeze(3).permute(0,2,1)
        #save_obj_mesh_with_rgb("mask.obj", face_shape[0], self.face_buf, skin_mask_3d[0])
        # save_image(skin_mask, "mask.png")
        
        # exit(0)
        return face_vertex, pred_texture, im_texture, pred_color, im_color, landmark, face_norm_roted, coef_dict['gamma'], skin_mask_3d
