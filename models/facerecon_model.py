...
class FaceReconModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--bfm_model', type=str, default='fuse_model_front.mat', help='bfm model')
        ...
        if is_train:
            ...
            parser.add_argument('--w_penalty', type=float, default=0.5, help='weight for penalty')
    ...
    def __init__(self, opt):
        ...
        self.model_names = ['net_recon', 'fuse_tex'] 
        self.parallel_names = self.model_names + ['renderer']
        self.tex_from_im = opt.tex_from_im


        self.facemodel = ParametricFaceModel(
            bfm_folder=opt.bfm_folder, camera_distance=opt.camera_d,  focal=opt.focal, center=opt.center,
            is_train=self.isTrain, cartoon_weight_shape=opt.cartoon_weight_shape, \
            cartoon_weight_tex=opt.cartoon_weight_tex, default_name=opt.bfm_model, tex_from_im=opt.tex_from_im
        )

        self.net_recon = networks.define_net_recon(
            net_recon=opt.net_recon, face_model=self.facemodel, use_last_fc=opt.use_last_fc, init_path=opt.init_path
        )

        self.fuse_tex = networks.fuse_texture(128 * 6)
        ...
        if self.isTrain:
            self.loss_names = ['all', 'feat', 'color', 'lm', 'reg', 'gamma', 'reflc', 'penalty']
            ...
            if self.tex_from_im:
                self.optimizer = torch.optim.Adam(self.fuse_tex.parameters(), lr=opt.lr)
            elif self.opt.head_only:
                self.optimizer = torch.optim.Adam(list(self.net_recon.final_layers2.parameters()) + list(self.fuse_tex.parameters()), lr=opt.lr)
            else:
                self.optimizer = torch.optim.Adam(list(self.net_recon.parameters()) + list(self.fuse_tex.parameters()), lr=opt.lr)
    ...
    def forward(self):
        ...
        self.pred_vertex, pred_tex, im_tex, pred_color, im_color, self.pred_lm, face_norm_roted, gamma, attn_mask = \
            self.facemodel.compute_for_render(output_coeff, self.input_img, self.atten_mask)
        ...
        self.pred_color = pred_tex
        #self.pred_color = im_tex
        #self.pred_color = attn_mask


        self.pred_mask, _, self.pred_face = self.renderer(
            self.pred_vertex, self.facemodel.face_buf, feat=self.pred_color.contiguous())
