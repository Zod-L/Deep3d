...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        # BFM setting
        parser.add_argument('--cartoon_weight_shape', type=float, default=0.5, help='Weight for cartoon mean face')
        parser.add_argument('--cartoon_weight_tex', type=float, default=0.5, help='Weight for cartoon mean face')
        parser.add_argument('--tex_from_im', type=lambda x: (str(x).lower() == 'true'), default=False, help='Whether to extract texture from image')
    ...
    def parse(self):
        ...
        if opt.phase != 'test':
            ...
            if opt.continue_train and opt.epoch_count is None:
                if opt.epoch == 'latest':
